...
- [2026-04-09] All S3 operations in `MediasStorageService` must be wrapped in `withRetry()`, including stream-based ones like `listObjects` — wrap the entire stream-to-Promise conversion inside the `withRetry` factory function so transient errors are retried correctly.
